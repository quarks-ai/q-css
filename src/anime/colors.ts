import { rgbExecRgx, hslExecRgx, hslaExecRgx } from "./consts";

import { round, is } from "./helpers";

// RGB / RGBA -> RGBA

function rgbToRgba(rgbValue: string) {
...
function hexToRgba(hexValue: any) {
	const hexLength = hexValue.length;
	const isShort = hexLength === 4 || hexLength === 5;
	const isAlpha = hexLength === 5 || hexLength === 9;
	const hexPrefix: any = "0x";
...
function hslToRgba(hslValue: any) {
	const hsl: any = hslExecRgx.exec(hslValue) || hslaExecRgx.exec(hslValue);
	const h = hsl[1] / 360;
	const s = hsl[2] / 100;
	const l = hsl[3] / 100;
	const a = hsl[4] || 1;
...
function normalizeColorToRgba(colorValue: any) {
