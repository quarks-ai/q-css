...
import { selectString, is } from "./helpers";
...
function getCircleLength(el: any) {
	return pi * 2 * el.getAttribute("r");
}

function getRectLength(el: any) {
	return el.getAttribute("width") * 2 + el.getAttribute("height") * 2;
}

function getLineLength(el: any) {
	return getDistance(
...
function getPolylineLength(el: any) {
	const points = el.points;
	if (is.und(points)) return 0;
	let totalLength = 0;
	let previousPos;
...
function getPolygonLength(el: any) {
	const points = el.points;
...
function setDashoffset(el: any) {
	const pathLength = getTotalLength(el);
	el.setAttribute("stroke-dasharray", pathLength);
	return pathLength;
}

function getParentSvgEl(el: any) {
...
function getParentSvg(pathEl: any, svgData: any = {}) {
	const svg = svgData;
...
function getPath(path: any, percent: any) {
...
function getPathPoint(pathEl: any, progress: any, offset = 0) {
